package OpenSSL::Versions::Table;
...
our @EXPORT = qw();
our @EXPORT_OK = qw( lookup_openssl_version_number );

{
    ...
    sub lookup_openssl_version_number {
        my ($hex_str) = @_;

        my $version_str;

        eval {
            $version_str = $table{ $hex_str };
        };

        return $version_str if defined $version_str;
        croak "Cannot find '$hex_str' in lookup table";
    }
}
